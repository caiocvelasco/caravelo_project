...
- **dbt_transformation (your-dbt-project)/**
    ...
    - **models**
      - **raw**
        - amadeus_flat_dbt.sql
        - sabre_flat_dbt.sql
        - vueling_flat_dbt.sql
      ...
      - **analytics**
        ...
        - dim_client.sql
        - fct_mrr_alloc.sql
        - mart_customer__mrr_monthly.sql
        - mart_internal__mrr_monthly_eur.sql
        - vw_bookings_summary.sql
    ...
    * `models/` folder: 
      * Contains the dbt models. The models can be located in folders or subfolders.  
        * `models/raw/snowflake_external_stage.yml`
          ...
          * The `dbt run-operation stage_external_sources` command (see below) will look into this file, which creates the External Tables in Snowflake in the RAW schema in Snowflake (*it already infers the schema from the JSON that is created in the VALUE column of each External Table*). 
          * When referencing these "sources" external tables in the dbt models, make sure to use the `{{ source('source_name','table_name') }}` jinja. The Staging and Analytics layers will make reference to the RAW models by using another jinja, the `{{ ref('raw_model_name') }}`.
          * Notice that the `source_name` is defined with the `name:` tag in the `snowflake_external_stage.yml` file.
        * `models/raw/amadeus_flat_dbt.sql`, `sabre_flat_dbt.sql`, `vueling_flat_dbt.sql`
          * These are the flattened views that process the external table data for each source system.
      * `models/staging`: Contains the dbt models to be materialized as tables in the STAGING schema in Snowflake.
        * `stg_bookings.sql`: Unified booking data from all sources at passenger × segment grain
        * `stg_clients.sql`, `stg_fx_rates.sql`, `stg_subscription_events.sql`: Additional staging models
      * `models/analytics`: Contains the dbt models to be materialized as views in the ANALYTICS schema in Snowflake.
        * `dim_bookings.sql`: Master dimension table for booking-level information
        * `vw_bookings_summary.sql`: Business-ready summary view
        * Additional marts and fact tables for comprehensive analytics
    * `macro/` folder:
      * Examples:
          ...
          * `macro/tests/`: Contains custom test macros for data quality validation. 
...
4) Run dbt for the second dbt project (`dbt_transformation/`)
  ...
  * run: `dbt run` and dbt will materialize the data into Snowflake's STAGING and ANALYTICS schemas.